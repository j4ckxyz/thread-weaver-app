    ...
    `;
        ...
        try {
            ...
            if (Array.isArray(parsed)) {
                // If the LLM returned just an array, assume it's a list of topics or nodes
                // We'll wrap it to satisfy our expected interface
                // Heuristic: check first item keys
                const first = parsed[0];
                if (first && (first.topics || first.title)) {
                    return { topics: parsed, nodes: [] };
                }
            }

            return parsed;
        } catch (e) {
            console.error('Failed to parse LLM JSON:', data.choices[0].message.content);
