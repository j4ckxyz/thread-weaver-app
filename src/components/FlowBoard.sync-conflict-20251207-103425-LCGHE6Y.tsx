...
export const FlowBoard: React.FC = () => {
    ...
    const [density, setDensity] = React.useState(100); // 0 to 100 percentage of nodes to show
    ...
    const visibleNodes = React.useMemo(() => {
        if (density === 100) return flowData.nodes;

        const essentialNodes = flowData.nodes.filter(n => n.type !== 'comment');
        const commentNodes = flowData.nodes.filter(n => n.type === 'comment');

        const countToShow = Math.floor(commentNodes.length * (density / 100));
        const visibleComments = commentNodes.slice(0, countToShow);

        return [...essentialNodes, ...visibleComments];
    }, [flowData.nodes, density]);

    const visibleEdges = React.useMemo(() => {
        const visibleNodeIds = new Set(visibleNodes.map(n => n.id));
        return flowData.edges.filter(e => visibleNodeIds.has(e.source) && visibleNodeIds.has(e.target));
    }, [flowData.edges, visibleNodes]);

    return (
        <div className={`transition-all duration-500 ease-in-out ${isFullscreen ? 'fixed inset-0 z-50 bg-zinc-950' : 'w-full h-[600px] rounded-2xl mt-8'} overflow-hidden glass-panel border border-zinc-700/50 animate-fade-in relative group`}>
            ...
            <ReactFlow
                key={`flow-${flowData.nodes.length}-${density}`}
                nodes={visibleNodes}
                edges={visibleEdges}
                nodeTypes={nodeTypes}
                fitView
